...
import { statefulConversationWorkflow, processUserMessage, getHistory } from '../workflows';

describe('openai-agents/stateful-conversation workflow', function () {
  this.timeout(30_000);
  ...
  it('drives two updates, hands off to a specialist, and exposes history via query', async () => {
    const taskQueue = 'test-stateful-conversation';
    ...
    await worker.runUntil(async () => {
      const handle = await testEnv.client.workflow.start(statefulConversationWorkflow, {
        workflowId: 'test-stateful-conversation-' + Date.now(),
        taskQueue,
      });
