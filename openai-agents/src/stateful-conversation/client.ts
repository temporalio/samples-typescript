...
async function run() {
  ...
  const workflowId = 'openai-agents-stateful-conversation-' + nanoid();
  const handle = await client.workflow.start(statefulConversationWorkflow, {
    taskQueue: 'openai-agents-stateful-conversation',
    workflowId,
  });
